...
const Objeto = () => {
    ...
    const startTimer = (e) => {
        ...
        var texto6 = "movil,silla,vaso,mesa,mechero,cigarro,botella,zapatilla,boligrafo,lapiz,papel,cuadro,ventana,reloj,mascarilla,bolso,riñonera,camiseta,pantalon,cargador,carta,balon,botas,paraguas,gafas,peine,cuerda,puente,casa,mochila,caja,mueble,escalera,altavoz,calendario,mando,television,microondas,horno,arco,flecha,espada,escudo,lanza,armadura,casco,moto,coche,camion,bicicleta,patinete,furgoneta,ordenador,tablet,consola,libro,cuaderno,carpeta,mascara,hoz,martillo,pico,pala,funda,kimono,chanclas,hacha,ballesta,farola,jabalina,figura,estatua,hoja,pañuelos,teclado,raton,brujula,armario,cajón,taza,caliz,armanaque,taxi,silla de ruedas,tubo,señal,cubo,bolsa,cenicerotapón,toallas,cepillo,fregona,juguete,llave inglesa,llave,sacapuntas,espantapajaros,tractor,goma";
        if(  document.getElementById("tiempo6").innerHTML== "58"){
           
            var opciones6 = texto6.split(",");
            var posicionAleatoria6 = Math.floor(Math.random() * opciones6.length);
            document.getElementById("palabra6").innerHTML =opciones6[posicionAleatoria6];
        }

        if(  document.getElementById("tiempo6").innerHTML== "50"){
        ...
        if(  document.getElementById("tiempo6").innerHTML== "40"){
         
            var opciones6 = texto6.split(",");
            var posicionAleatoria6 = Math.floor(Math.random() * opciones6.length);
            document.getElementById("palabra6").innerHTML =opciones6[posicionAleatoria6];
        }
        if(  document.getElementById("tiempo6").innerHTML== "30"){
            
            var opciones6 = texto6.split(",");
            var posicionAleatoria6 = Math.floor(Math.random() * opciones6.length);
            document.getElementById("palabra6").innerHTML =opciones6[posicionAleatoria6];
        }
        if(  document.getElementById("tiempo6").innerHTML== "20"){
           
            var opciones6 = texto6.split(",");
            var posicionAleatoria6 = Math.floor(Math.random() * opciones6.length);
            document.getElementById("palabra6").innerHTML =opciones6[posicionAleatoria6];
        }
        if(  document.getElementById("tiempo6").innerHTML== "10"){
    ...
    return(
        <div class="container">
       <h2 class="my-3 h2">Objetos</h2>
       <h2 id="tiempo6">{timer}</h2>
       <p id="palabra6"></p>
